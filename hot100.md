...
- [\[字符串|滑动窗口\] 76.最小覆盖子串](#字符串滑动窗口-76最小覆盖子串)
...
- [\[数组|集合\] 349. 两个数组的交集](#数组集合-349-两个数组的交集)
...
var lengthOfLongestSubstring = function (s) {
    ...
    // 初始化滑动窗口的左右指针：左指针是外层遍历的指针，右指针从0开始根据条件向后移动
    // 依次遍历，找每一个元素的最大滑动窗口
...
```
- new Map 是可迭代的可以用 for of 遍历，字面法 {}/Object 是不可迭代的
- 跳出 for 循环的方式：
    1. break 跳出循环但不跳出方法
    2. continue 跳出当次循环
    3. return 直接跳出方法并返回值
- 滑动窗口一般写法：
    外层是一个指针的移动及边界条件，
    内层是另一个指针（注意内层指针的边界在内层逻辑中写不然容易死循环）
- 方法二（TODO 之后再战，写这个方法一被折磨了两天）：
    外层右指针 内层左指针，
    记录最小子串的起点终点位置，
    找到符合条件的子串之后比较下标位置判断是否更小，
    最终直接通过 substring() 方法截取字符串即可
```

```javascript
/**
 * 方法一：外层左指针，内层右指针，记录最小子串
 *  时间复杂度O(m+n) 左右指针都移动到字符串最末端
 *  空间复杂度O(n)
 * @param {string} s
 * @param {string} t
 * @return {string}
...
var minWindow = function (s, t) {
    let m1 = new Map(), // t字符串字符统计
        m2 = new Map(), // 当前窗口字符串字符统计
        min = '', // 当前最小子串
        minLen = Infinity, // 初始化为正无穷
        curr = ''; // 当前窗口的字符串

    // 统计t字符串每一个字符及出现次数
    for (let c of t) {
        m1.set(c, m1.get(c) ? m1.get(c) + 1 : 1);
    }

    // 滑动窗口，外层是左指针的移动，内层是右指针移动
    for (let l = 0, r = 0; l < s.length; l++) {
        while (!isCovered(curr) && r < s.length) {
            curr += s[r];
            m2.set(s[r], m2.get(s[r]) ? m2.get(s[r]) + 1 : 1);
            r++;
        }
        if (isCovered(curr)) {
            // 比较是否是更小解
            ...
            // 抛弃第一个字符
            let f = curr.charAt(0);
            m2.set(f, m2.get(f) ? m2.get(f) - 1 : 0);
            curr = curr.slice(1);
        } else {
            // 右移到最右侧也不满足，直接跳出循环
            break;
 ...
 * 核心关键写法：
 * 
 * 上面写循环是因为leetcode上要求输出的格式是数组套数组，
 * 把每层节点都放一数组里，
 * 如果就直接输出非常简单，如下：
 */
...
```
1. new Set(Iterator) 可以直接将可迭代对象不重复的放入set
```

```javascript
/**
 * 方法一：Set集合【推荐】
 *  - 将其中一个放入Set，直接遍历另一个数组，集合中存在则放入并删除（避免重复加入相同元素）
 * 
 *  时间复杂度 O(m+n)，set遍历nums1，循环nums2，查找setO(1)，综合下来是O(m+n)
 *  空间复杂度 O(m+n)，set占用空间O(m)，res占据 O(n)
 * 
 * @param {number[]} nums1
 * @param {number[]} nums2
 * @return {number[]}
...
/**
 * 方法二：排序+双指针
 *  先将两个数组从小到大排序，利用两个指针分别遍历两个数组，比较是否相同决定是否放入结果数组中
 *  注意去重，相同不放入，但是要向后移动指针
 * 
 *  > sort是原地排序并返回当前引用，另外别省略排序实现，不然可能会产生问题
 * 
 *  时间复杂度：O(mlogm + nlogn)
 *      - 排序复杂度分别为 O(mlogm) 和 O(nlogn)
 *      - 双指针遍历复杂度 O(min(m,n))
 *  空间复杂度 O(logm + logn) 排序所用的额外空间
 * 
 * @param {number[]} nums1
 * @param {number[]} nums2
 * @return {number[]}
...
var intersection = function (nums1, nums2) {
    ...
    let i = 0, j = 0, res = [], l1 = nums1.length, l2 = nums2.length;
    while (i < l1 && j < l2) {
        if (nums1[i] === nums2[j]) {
            // 与上一次被放入的值比较保证元素唯一性
